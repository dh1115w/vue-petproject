...
<div class="clinic-container main-container page-enter">
  ...
  <div class="clinic-banner">
    ...
    <img src="@/images/clinic-search-banner.jpg" alt="附近診所 Banner" class="banner-image" />
            ...
            >
              <div class="clinic-card-top flex">
                <div 
                  ...
                  :class="{ 'badge-open': clinic.isOpen, 'badge-closed': !clinic.isOpen }"
                >
                  📍
                </div>
                ...
                <div class="clinic-info-main">
                  ...
                  <div class="clinic-meta-row">
                    🕒 {{ clinic.hours }} <span class="divider-dot">·</span> 📍 {{ clinic.distance }}
                  </div>

                  <div class="clinic-rating-row">
                    ⭐ <span class="rating-num">{{ clinic.rating }}</span> 
                    <span class="reviews-count">({{ clinic.reviews }} 則評價)</span>
                  </div>

                  <div class="clinic-tags-wrapper">
                    <span v-for="tag in clinic.tags" :key="tag" class="normal-tag">
                      {{ tag }}
                    </span>
                    <span v-if="clinic.isNight" class="night-tag">
                      夜間
                    </span>
                  </div>
                </div>
              ...
              <div class="clinic-card-actions">
                <button class="action-nav-btn" @click.stop="handleNavigate(clinic)">
                  🚀 導航
                </button>
                <button class="action-phone-btn" @click.stop="handleCall(clinic)">
                  📞 撥號
                </button>
              </div>
            </div>
...
const filteredClinics = computed(() => {
  return clinicsDatabase.value.filter((c) => {
    ...
    const matchSearch = c.name.includes(searchQuery.value) || c.address.includes(searchQuery.value)
