<template>
  ...
  <div class="tracking-container page-container page-enter">
    ...
    <div class="health-tracking-banner">
      <img src="@/images/health-tracking-banner.jpg" alt="健康追蹤 Banner" class="banner-image" />
        ...
        <div class="status-summary-cards-grid">
          ...
          >
            <div class="summary-card-top">
              <div class="summary-icon-box" :style="{ backgroundColor: stat.bgColor, color: stat.color }">
                {{ stat.emoji }}
              </div>
              <span v-if="stat.trend === 'up'" class="trend-indicator text-success">📈</span>
              <span v-else-if="stat.trend === 'down'" class="trend-indicator text-danger">📉</span>
              <span v-else-if="stat.trend === 'neutral'" class="trend-indicator text-muted">➖</span>
        ...
        <div class="pawcare-card chart-container-card" style="margin-top: 1rem;">
          ...
          <div class="chart-visual-wrapper">
            <svg class="line-chart-svg" viewBox="0 0 500 180">
              <!-- 背景參考虛線網格、偏瘦與過重臨界警示線 -->
              <line x1="40" y1="30" x2="480" y2="30" stroke="#df4733" stroke-dasharray="4 4" opacity="0.4" />
              <text x="485" y="34" font-size="10" fill="#df4733">過重(16kg)</text>

              <line x1="40" y1="130" x2="480" y2="130" stroke="#e29553" stroke-dasharray="4 4" opacity="0.4" />
              <text x="485" y="134" font-size="10" fill="#e29553">偏瘦(11kg)</text>
              
              <line x1="40" y1="80" x2="480" y2="80" stroke="#e6e3da" stroke-dasharray="3 3" />
              ...
              <!-- 折線上的動態數據核心圓點群 -->
              <circle cx="50" cy="110" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="110" cy="95" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="170" cy="100" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="230" cy="85" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="290" cy="90" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="350" cy="75" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="410" cy="80" r="4" fill="var(--primary)" stroke="#fff" stroke-width="2" />
              <circle cx="470" cy="70" r="5" fill="#7bb3d4" stroke="#fff" stroke-width="2" /> <!-- 點中的最新體重節點 -->

              <!-- X 軸底部日期文字刻度 -->
              <text v-for="(d, idx) in ['3/1', '3/8', '3/15', '3/22', '3/29', '4/5', '4/12', '4/19']" 
                    :key="idx" :x="50 + idx*60" y="165" text-anchor="middle" font-size="11" fill="#9c9e98">
                {{ d }}
              </text>
            </svg>
          </div>
        ...
        <div class="pawcare-card chart-container-card" style="margin-top: 1rem;">
          <div class="chart-header-row">
            ...
            <div class="bar-chart-legend">
              <span class="legend-badge-item"><i class="dot-g"></i> 飲水(ml)</span>
              <span class="legend-badge-item"><i class="dot-o"></i> 進食(g)</span>
            </div>
          </div>
          ...
          <div class="bar-chart-visual-wrapper">
            ...
            <div class="bar-chart-main-content">
              <!-- 跑迴圈依據數據比例動態長出長條柱 -->
              <div v-for="day in weeklyFoodWaterData" :key="day.date" class="bar-column-group">
                <div class="bars-two-mix-stack">
                  <!-- 飲水柱狀體（綠色） -->
                  <div class="single-bar water-bar" :style="{ height: (day.water / 650) * 100 + '%' }">
                    <span class="bar-tooltip-val">{{ day.water }}ml</span>
                  </div>
                  <!-- 進食柱狀體（橘色） -->
                  <div class="single-bar food-bar" :style="{ height: (day.food / 650) * 100 + '%' }">
...
const monthlyData = [
  { date: "3/1", weight: 12.8 }, { date: "3/8", weight: 13.1 },
  { date: "3/15", weight: 13.0 }, { date: "3/22", weight: 13.3 },
  { date: "3/29", weight: 13.2 }, { date: "4/5", weight: 13.5 },
  { date: "4/12", weight: 13.4 }, { date: "4/19", weight: 13.6 },
]

const yearlyData = [
  { date: "1月", weight: 11.2 }, { date: "2月", weight: 11.8 },
  { date: "3月", weight: 12.3 }, { date: "4月", weight: 12.8 },
  { date: "5月", weight: 13.1 }, { date: "6月", weight: 13.0 },
  { date: "7月", weight: 13.2 }, { date: "8月", weight: 13.4 },
  { date: "9月", weight: 13.3 }, { date: "10月", weight: 13.5 },
  { date: "11月", weight: 13.6 }, { date: "12月", weight: 13.6 },
]
...
const statusLabel = computed(() => {
  return currentWeight < idealMin ? "偏瘦" : currentWeight > idealMax ? "過重" : "正常"
})

const statusColor = computed(() => {
  return currentWeight < idealMin ? "#e29553" : currentWeight > idealMax ? "#df4733" : "#6bae8a"
})

const statusBg = computed(() => {
  return currentWeight < idealMin ? "rgba(226, 149, 83, 0.15)" : currentWeight > idealMax ? "rgba(223, 71, 51, 0.15)" : "rgba(107, 174, 138, 0.15)"
})

// 右側上方：三大健康數據摘要小卡片的聯動資料
const healthStatsSummary = computed(() => {
  return [
    { label: "當前體重", value: currentWeight + " kg", sub: "上週 +0.1kg", emoji: "⚖️", color: "#7bb3d4", bgColor: "rgba(123, 179, 212, 0.15)", trend: "up" },
    { label: "飲水達成率", value: "80%", sub: "480/600 ml", emoji: "💧", color: "#6bae8a", bgColor: "rgba(107, 174, 138, 0.15)", trend: "neutral" },
    { label: "體重狀態", value: statusLabel.value, sub: "理想範圍內", emoji: "🐕", color: statusColor.value, bgColor: statusBg.value, trend: "ok" }
  ]
})
