...
<div class="records-container page-enter">
  ...
  <div class="medical-records-banner">
    ...
    <img src="@/images/medical-records-banner.jpg" alt="病歷紀錄 Banner" class="banner-image" />
      ...
      <div class="left-panel-column">
        ...
        <div class="pawcare-card illustration-card">
          ...
          <div class="illustration-footer-info">
            <p>共 {{ recordsDatabase.length }} 筆病歷 · {{ totalFilesCount }} 個檔案</p>
          </div>
        </div>
        ...
        <div class="records-list-stack">
          ...
          >
            ...
            >
              ...
              <div class="record-type-icon" :style="{ backgroundColor: getEventConfig(record.type).bg }">
              ...
              <div class="record-info-center">
                <div class="record-title-row">
                  ...
                  <span 
                    ...
                    :style="{ backgroundColor: getEventConfig(record.type).bg, color: getEventConfig(record.type).color }"
                  >
                ...
                <div class="record-tags-box">
                  <span v-for="tag in record.tags" :key="tag" class="record-mini-tag">
                    {{ tag }}
                  </span>
                </div>
              </div>
              ...
              <div class="record-header-right">
                <span 
                  :class="(record.status === '正常' || record.status === '完成' || record.status === '已康復') ? 'status-badge-normal' : 'status-badge-warning'"
                >
                  {{ record.status }}
                </span>
            ...
            <div v-if="expandedRecordId === record.id" class="record-expanded-drawer">
              ...
              <div class="drawer-actions-row">
                <button class="drawer-view-btn" @click="handleViewFile(record)">👁️ 查看</button>
                <button class="drawer-download-btn" @click="handleDownloadFile(record)">📥 下載</button>
              </div>
            </div>
...
// ==========================================================================
const typeConfig = {
  checkup: { label: "健康檢查", icon: "🩺", color: "#7bb3d4", bg: "rgba(123, 179, 212, 0.12)" },
  vaccine: { label: "疫苗接種", icon: "💉", color: "#6bae8a", bg: "rgba(107, 174, 138, 0.12)" },
  medicine: { label: "用藥紀錄", icon: "💊", color: "#e29553", bg: "rgba(226, 149, 83, 0.12)" },
  followup: { label: "回診追蹤", icon: "📄", color: "#b59bd4", bg: "rgba(181, 155, 212, 0.12)" },
  emergency: { label: "急診紀錄", icon: "🚨", color: "#df4733", bg: "rgba(223, 71, 51, 0.12)" },
}
...
const filteredRecords = computed(() => {
  return recordsDatabase.value.filter((record) => {
    ...
    const matchSearch = record.title.includes(searchQuery.value) || 
                        record.clinic.includes(searchQuery.value) || 
                        record.tags.some((t) => t.includes(searchQuery.value))
    
    // 2. 類別分頁切換過濾
    const matchType = selectedTypeKey.value === 'all' || record.type === selectedTypeKey.value
...
function handleRecordInfo() { alert("💡 新增病歷功能即將推出！"); }
function handleFileSelect() { alert("請選擇您電腦中的 PDF 或圖檔報告 📄"); }
...
function handleViewFile(record) { alert(`👁️ 正在為您開啟：\n【${record.title}】的 PDF 報告原始檔\n主治醫生：${record.doctor}`); }
function handleDownloadFile(record) { alert(`📥 開始下載【${record.title}】的病歷打包檔案 (共 ${record.files} 個附件)`); }
